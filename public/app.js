...
var Firebase, URL, URL0, URL1, URL2, _, app, contactFirebase, ref, respond, response;
...
URL0 = 'https://ctpaint-pre-order';

URL1 = 'ebas';

URL = URL0 + '.fir' + URL1 + URL2;

app = Elm.Main.fullscreen();

ref = app.ports, contactFirebase = ref.contactFirebase, response = ref.response;

respond = function(email) {
  return app.ports.response.send(email);
};

contactFirebase.subscribe(function(email) {
  var email_, fb, hasADot, hasAtSign;
  hasAtSign = (email.indexOf('@')) !== -1;
  hasADot = (email.indexOf('.')) !== -1;
  if (hasADot && hasAtSign) {
    email_ = email.split('');
    email_ = _.map(email, function(ch, i) {
      if (ch === '@') {
        ch = ' at ';
      }
      if (ch === '.') {
    ...
    email_ = _.reduce(email_, function(sum, ch) {
      return sum + ch;
    }, '');
    fb = new Firebase(URL + '/' + email_);
    return fb.set(email, function(error) {
      if (error) {
